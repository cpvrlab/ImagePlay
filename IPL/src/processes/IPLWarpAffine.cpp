...
void IPLWarpAffine::init()
{
    ...
    addProcessPropertyVectorDouble("matrix_M", "M:2|3", "Transformation Matrix", value, IPL_WIDGET_MATRIX);
...
bool IPLWarpAffine::processInputData(IPLData* data, int imageIndex, bool)
{
    ...
    cv::Point2f pointsA[3];
    cv::Point2f pointsB[3];
    ...
    if(method == 1)
    {
        matrix = cv::getAffineTransform(pointsB, pointsA);
    }

    cv::Mat result;
    cv::warpAffine(_inputB->toCvMat(), result, matrix, cv::Size(image->width(), image->height()), interpolation, border);
...
IPLData* IPLWarpAffine::getResultData(int index)
{
    switch (index) {
    case 1:
        return _previewA;
